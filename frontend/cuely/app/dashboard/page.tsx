...
export default function DashboardPage() {
    ...
    const fetchSessions = async () => {
        try {
            ...
            if (sessionsData.success) {
                ...
                calculateStats(sessionsData.data, userData.success ? userData.data : null);
            }
        } catch (err) {
    ...
    const calculateStats = (sessionList: Session[], userData: any | null) => {
        const total = sessionList.length;
        const complete = sessionList.filter(s => s.completedAt).length;
        const rate = total > 0 ? (complete / total) * 100 : 0;

        // Use real backend calculated score if available, otherwise fallback (0)
        const avgScore = userData?.stats?.overallEmpathyScore || 0;
